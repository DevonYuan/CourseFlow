...
## Requirements
### Functional
- Define the three-process model: **Main**, **Preload**, **Renderer** — responsibilities and boundaries
- Define the **IPC schema** (channel names, request/response payloads, event names) as TypeScript types in `src/shared/ipc.ts`
...
- Shared code (`src/shared/`) has **zero** Electron/Node dependencies
...
|---------|------------------|-----------|
| **Main** | Window lifecycle, native menus, dialogs, auto-updater, SQLite DB (single connection), iCal network fetch, IPC handlers, app protocol handling | React, DOM, CSS, Vite HMR |
| **Preload** | `contextBridge.exposeInMainWorld('api', { ... })` — typed wrappers around `ipcRenderer.invoke/on` | Business logic, DB access, network |
| **Renderer** | React UI, Zustand stores, user interactions, calls `window.api.*` | `require`, `process`, `fs`, `net`, `electron` |
...
### TypeScript Contract (src/shared/ipc.ts)
```ts
...
- `src/shared/ipc.ts` (TypeScript interfaces above)
- `src/shared/types.ts` (core domain types: `Assignment`, `SubTask`, `Note`, `PriorityOrder`, `Settings`, `ICalEvent`)

### Modified Files
- `src/preload/index.ts` — implement typed `contextBridge` exposure using `ipc.ts` types
- `src/main/ipc-handlers.ts` — skeleton handlers keyed by `ipc.ts` channels (throw `NotImplementedError` for now)
...
|---|-----------|--------------|
| 1 | `src/shared/ipc.ts` exports `IpcChannels` and `IpcEvents` with all planned Phase 0–1 channels | Code review |
| 2 | `src/shared/types.ts` exports all domain types used in IPC payloads | Code review |
| 3 | `src/preload/index.ts` exposes `window.api` with **full type safety** (no `any`) | `pnpm typecheck` passes |
| 4 | `src/main/ipc-handlers.ts` registers a handler for every `IpcChannels` key (stub OK) | Search for `ipcMain.handle` count matches channel count |
...
- The ADR documents *why* we chose this model (security, testability, maintainability).
...
> **Impact:** Zero user-visible change. All future tickets import from `src/shared/ipc.ts` and `src/shared/types.ts`.