...
function register<K extends keyof IpcChannels>(
  channel: K,
  handler: Handler<K>
) {
...
register('db:assignments:delete', (_, req) => { repo.deleteAssignment(req); });
register('db:priority:reorder', (_, req) => { repo.setPriorityOrder(req); });
