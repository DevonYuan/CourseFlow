...
- **Folder structure** (finalize the open item from `docs/tickets/phase0/README.md`):
  ...
  src/
    ...
    shared/        # Pure TS shared types/utilities (no Electron deps)
  ```
...
- `.prettierrc`
- `.husky/pre-commit`
- `src/main/index.ts` (minimal main process: create window, load renderer)
- `src/preload/index.ts` (minimal preload: expose nothing yet, contextBridge ready)
- `src/renderer/index.html` + `src/renderer/src/main.tsx` + `App.tsx` (React 18 + TSX)
- `src/shared/types.ts` (empty barrel export)
