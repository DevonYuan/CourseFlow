...
export const useAssignmentsStore = create()((set, get) => ({
    ...
    setAssignmentStatus: (id, status) => {
        set((state) => ({
            assignments: state.assignments.map((assignment) => assignment.id === id ? { ...assignment, status } : assignment),
        }));
    },
 ...
 */
export function initializeAssignmentsStore() {
    const store = useAssignmentsStore.getState();
    store.fetchAssignments();
    return () => { };
}
//# sourceMappingURL=assignmentsStore.js.map