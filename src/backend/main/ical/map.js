 ...
 */
export function generateCourseColor(courseName) {
    let hash = 0;
 ...
 */
export function extractCourseName(event) {
 ...
 */
function calculateNumericPriority(dueDateIso, now = Date.now()) {
    if (!dueDateIso)
        return 999;
 ...
 */
function mapPriorityToEnum(numericPriority) {
 ...
 * ```
 */
export function mapICalToAssignments(events, sourceUrl) {
    const now = Date.now();
