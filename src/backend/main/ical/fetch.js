 ...
 */
export async function fetchICalFeed(url, options = {}) {
        ...
        try {
            ...
            if (response && typeof response.ok === 'boolean') {
                if (response.ok === false) {
                    const error = new HttpError(`HTTP ${response.status} ${response.statusText} fetching iCal feed: ${sanitizedUrl} (attempt ${attempt}/${config.maxRetries})`, response.status);
                    ...
                    if (!contentType.includes('text/calendar') && !contentType.includes('text/plain')) {
                        console.warn(`[iCal Fetch] Unexpected content-type "${contentType}" for ${sanitizedUrl} (attempt ${attempt})`);
                    }
            ...
            if (error instanceof DOMException && error.name === 'AbortError') {
                const timeoutError = new TimeoutError(`Request timeout after ${config.timeoutMs}ms fetching iCal feed: ${sanitizedUrl} (attempt ${attempt}/${config.maxRetries})`, error);
                lastError = timeoutError;
            }
            else if (error instanceof TypeError) {
                // Network error (DNS, connection refused, etc.) or invalid response
                const networkError = new NetworkError(`Network error fetching iCal feed: ${sanitizedUrl} (attempt ${attempt}/${config.maxRetries}): ${error.message}`, error);
                ...
                // Unexpected error
                const unexpectedError = new ICalFetchError(`Unexpected error fetching iCal feed: ${sanitizedUrl} (attempt ${attempt}/${config.maxRetries}): ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error : undefined);
    ...
    if (lastError instanceof TimeoutError) {
        throw new TimeoutError(`Request timeout after ${config.maxRetries} attempts (${config.timeoutMs}ms each) fetching iCal feed: ${sanitizedUrl}`, lastError);
    }
    if (lastError instanceof NetworkError) {
        throw new NetworkError(`Network error after ${config.maxRetries} attempts fetching iCal feed: ${sanitizedUrl}`, lastError);
    }
    if (lastError instanceof ICalFetchError) {
        throw lastError;
    }
    throw new ICalFetchError(`Failed to fetch iCal feed after ${config.maxRetries} attempts: ${sanitizedUrl}`, lastError);
}
/**
 ...
 */
function flushProperty(currentEvent, currentProperty, currentValue) {
 ...
 * @returns Array of parsed ICalEvent objects
 */
export async function fetchAndParseICalFeed(url, options) {
