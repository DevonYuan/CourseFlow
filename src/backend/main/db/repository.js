...
import { mapDbAssignmentToAssignment, mapAssignmentInputToDb, mapDbSubTaskToSubTask, mapSubTaskInputToDb, mapDbSettingsToSettings, toIsoDateTime, toUnixMs, } from './mappers.js';
// ============================================================================
// SQL Helpers
// ============================================================================
...
export const repo = {
     ...
     */
    listAssignments() {
        const rows = all('SELECT * FROM assignments ORDER BY due_at ASC');
     ...
     */
    getAssignment(id) {
        const row = get('SELECT * FROM assignments WHERE id = ?', [id]);
     ...
     */
    findByICalUID(uid) {
        const row = get('SELECT * FROM assignments WHERE ical_uid = ?', [uid]);
        return row ? mapDbAssignmentToAssignment(row) : null;
    },
    /**
     * List assignments for a specific course.
     */
    listAssignmentsByCourse(courseName) {
        const rows = all('SELECT * FROM assignments WHERE course_name = ? ORDER BY due_at ASC', [courseName]);
        return rows.map(mapDbAssignmentToAssignment);
    },
    /**
    ...
    `;
        ...
        const row = get('SELECT * FROM assignments WHERE id = ?', [id]);
        if (!row)
            throw new Error('Failed to retrieve upserted assignment');
     ...
     */
    getPriorityOrder() {
        const rows = all('SELECT assignment_id FROM priority_order ORDER BY position ASC');
     ...
     */
    listSubTasks(assignmentId) {
        const rows = all('SELECT * FROM sub_tasks WHERE assignment_id = ? ORDER BY position ASC', [assignmentId]);
    ...
    `, [
        ...
        const row = get('SELECT * FROM sub_tasks WHERE id = ?', [dbInput.id]);
        if (!row)
            throw new Error('Failed to retrieve upserted sub-task');
     ...
     */
    reorderSubTasks(assignmentId, ids) {
        const now = Date.now();
        exec('BEGIN TRANSACTION');
        try {
            for (let i = 0; i < ids.length; i++) {
     ...
     */
    getNote(assignmentId) {
        const row = get('SELECT * FROM notes WHERE assignment_id = ?', [assignmentId]);
        if (!row)
            return null;
        return {
    ...
    async setSetting(key, value) {
        ...
        run('INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value', [key, JSON.stringify(valueToStore)]);
    },
    /**
