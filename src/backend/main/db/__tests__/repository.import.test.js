...
function createAssignment(input) {
    ...
    assignmentCounter++;
    return {
        id: input.id ?? `assignment-${assignmentCounter}`,
        title: input.title ?? 'Test Assignment',
        description: input.description ?? '',
        courseId: input.courseId ?? `course-${assignmentCounter}`,
        courseName: input.courseName ?? 'CS 101',
        courseColor: input.courseColor ?? '#6366f1',
        dueAt: input.dueAt ?? new Date(now + 86400000).toISOString(),
        ...
        createdAt: input.createdAt ?? new Date(now - 86400000).toISOString(),
...
async function importAssignments(inputs) {
    if (!testDb)
  ...
  `);
    ...
    try {
        for (const input of inputs) {
            ...
            if (!existing) {
                // INSERT
                const id = input.id ?? `new-${Math.random().toString(36).slice(2)}`;
                ...
                const storedUpdatedAt = existing['updated_at'];
...
describe('importAssignments', () => {
        ...
        it('should count imported correctly for mixed batch', async () => {
            ...
            const inputs = [
                createAssignment({
                    ...
                    updatedAt: new Date(baseTime).toISOString(), // same updatedAt = skip
                }),
    ...
    describe('UPDATE scenarios', () => {
        it('should update when incoming updatedAt is newer', async () => {
            ...
            const existing = createAssignment({
                ...
                dueAt: new Date(baseTime + 86400000).toISOString(),
                updatedAt: new Date(baseTime).toISOString(),
            });
            await importAssignments([existing]);
            ...
            const updated = createAssignment({
                ...
                dueAt: new Date(baseTime + 172800000).toISOString(), // changed
                updatedAt: new Date(baseTime + 3600000).toISOString(), // 1 hour newer
            });
    ...
    describe('SKIP scenarios', () => {
        ...
        it('should skip when incoming updatedAt is older', async () => {
            ...
            const existing = createAssignment({
                ...
                title: 'Original',
                updatedAt: new Date(baseTime + 3600000).toISOString(), // newer
            });
            await importAssignments([existing]);
            ...
            const older = createAssignment({
                icalUid: 'skip-older@example.com',
                title: 'Modified (should be ignored)',
                updatedAt: new Date(baseTime).toISOString(), // older
            });
    ...
    describe('Protected fields preservation', () => {
        ...
        it('should preserve priority (stored in separate table)', async () => {
            ...
            const assignmentId = rows1[0]?.values?.[0]?.[0];
    ...
    describe('Mixed batch scenarios', () => {
        it('should return correct counts for mixed batch (3 new, 2 updated, 1 skipped)', async () => {
            ...
            await importAssignments([
                createAssignment({ icalUid: 'existing-1@example.com', title: 'Old 1', updatedAt: new Date(baseTime).toISOString() }),
                createAssignment({ icalUid: 'existing-2@example.com', title: 'Old 2', updatedAt: new Date(baseTime).toISOString() }),
                createAssignment({ icalUid: 'existing-3@example.com', title: 'Old 3', updatedAt: new Date(baseTime).toISOString() }),
            ]);
            // Import batch: 3 new, 2 updated (newer), 1 skipped (older)
            const inputs = [
                createAssignment({ icalUid: 'new-1@example.com', title: 'New 1', updatedAt: new Date(baseTime + 1000).toISOString() }),
                createAssignment({ icalUid: 'new-2@example.com', title: 'New 2', updatedAt: new Date(baseTime + 2000).toISOString() }),
                createAssignment({ icalUid: 'new-3@example.com', title: 'New 3', updatedAt: new Date(baseTime + 3000).toISOString() }),
                createAssignment({ icalUid: 'existing-1@example.com', title: 'Updated 1', updatedAt: new Date(baseTime + 3600000).toISOString() }), // newer
                createAssignment({ icalUid: 'existing-2@example.com', title: 'Updated 2', updatedAt: new Date(baseTime + 3600000).toISOString() }), // newer
                createAssignment({ icalUid: 'existing-3@example.com', title: 'Skipped 3', updatedAt: new Date(baseTime - 3600000).toISOString() }), // older
            ];
            const result = await importAssignments(inputs);
            expect(result.imported).toBe(3);
    ...
    describe('Transactional behavior', () => {
        it('should rollback all on constraint violation', async () => {
            ...
            await importAssignments([
                createAssignment({ icalUid: 'unique-1@example.com', courseId: 'canvas-1', updatedAt: new Date(baseTime).toISOString() }),
            ]);
            // Try to import batch with duplicate canvas_id (unique constraint)
            const inputs = [
                createAssignment({ icalUid: 'new-1@example.com', courseId: 'canvas-1', updatedAt: new Date(baseTime + 1000).toISOString() }), // duplicate canvas_id
                createAssignment({ icalUid: 'new-2@example.com', courseId: 'canvas-2', updatedAt: new Date(baseTime + 2000).toISOString() }),
            ];
...
//# sourceMappingURL=repository.import.test.js.map