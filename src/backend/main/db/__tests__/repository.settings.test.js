...
describe('Settings Repository', () => {
    ...
    describe('getAllSettings', () => {
        ...
        it('should merge stored settings with defaults', async () => {
            ...
            db.run('INSERT INTO settings (key, value) VALUES (?, ?), (?, ?)', ['theme', JSON.stringify('dark'), 'autoFetchIcal', JSON.stringify(true)]);
            saveDatabase();
            const settings = await repo.getAllSettings();
        ...
        it('should compute autoFetchIntervalMs from icalFetchIntervalMinutes', async () => {
            const db = getDatabase();
            db.run('INSERT INTO settings (key, value) VALUES (?, ?)', ['icalFetchIntervalMinutes', JSON.stringify(30)]);
            saveDatabase();
            const settings = await repo.getAllSettings();
    ...
    describe('resetSettings', () => {
        ...
        it('should clear all settings from database and re-insert defaults', async () => {
            ...
            const keys = [];
            while (rows.step()) {
                keys.push(rows.getAsObject()['key']);
            }
            rows.free();
    ...
    describe('encryption integration', () => {
        ...
        it('should handle corrupted encrypted data gracefully', async () => {
            ...
            const db = getDatabase();
            db.run('INSERT INTO settings (key, value) VALUES (?, ?)', ['icalUrl', JSON.stringify({ v: 1, ciphertext: 'bad', iv: 'bad', salt: 'bad' })]);
            saveDatabase();
...
//# sourceMappingURL=repository.settings.test.js.map